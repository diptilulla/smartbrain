...
class Signin extends React.Component { 
    ...
    onSubmitSignIn = () => {
        fetch('http://localhost:3001/signin', {   //fetch by default does get request but here we need post request so we add a second parameter that is an object to describe what eq would be
            method: 'post',
            ...
            .then(user => {
                if (user.id){      //we check if user has an id because if a new user signs in without register then we return a string as error which is considered tru inthis if statement so we check if user has id
